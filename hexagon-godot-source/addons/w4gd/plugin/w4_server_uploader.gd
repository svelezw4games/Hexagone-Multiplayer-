...
func _update_fleet_options() -> void:
	...
	var result = await _client.rest.GET("/fleet", {}, {'Accept-Profile': 'matchmaker'}).async()
	if result.is_error():
...
func _do_upload_and_create_build(p_build_name: String, p_fleet: Dictionary) -> bool:
	...
	result = await _client.rest.rpc("matchmaker.gameserver_build_create", data).async()
	if result.is_error():
	...
	if not p_fleet.is_empty():
		...
		result = await _client.rest.rpc("matchmaker.fleet_update", fleet).async()
		if result.is_error():

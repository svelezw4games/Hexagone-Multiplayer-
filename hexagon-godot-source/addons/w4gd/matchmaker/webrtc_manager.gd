...
func subscribe() -> void:
	if not _realtime_webrtc_channel:
		...
		_realtime_webrtc_channel.on_postgres_changes('*', 'matchmaker.webrtc_session', 'lobby_id=eq.' + _lobby_id)
...
func refresh_sessions() -> Request:
	var request = _client.rest.rpc("matchmaker.webrtc_sessions_for_player", {
		lobby_id = _lobby_id,
...
func _on_peer_session_description_created(p_type: String, p_sdp: String, p_peer_id: int) -> void:
	...
	if session.is_source:
		result = await _client.rest.rpc('matchmaker.webrtc_create_offer', {
			session_id = session.session_id,
			offer = description,
		}).async()
	else:
		result = await _client.rest.rpc('matchmaker.webrtc_create_answer', {
			session_id = session.session_id,
			answer = description,
		}).async()
...
func _send_queued_candidates(p_peer_id: int) -> void:
	...
	var result = await _client.rest.rpc('matchmaker.webrtc_add_candidates', data).async()

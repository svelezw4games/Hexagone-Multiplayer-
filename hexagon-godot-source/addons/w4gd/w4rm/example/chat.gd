...
class ChatMessage:
	...
	static func _w4rm_type_options(opts) -> void:
		opts['lobby_id'] = W4RM.tref("matchmaker.lobby", {
			external = true,
		})
	...
	static func _w4rm_security_policies(policies) -> void:
		policies["Authenticated users can view messages in lobbies they are in"] = \
			W4RM.build_security_policy_for('authenticated').can_select_if('matchmaker.user_in_lobby(lobby_id)');
		policies["Authenticated users can only post messages in lobbies they are in"] = \
			W4RM.build_security_policy_for('authenticated').can_insert_if('matchmaker.user_in_lobby(lobby_id)');
